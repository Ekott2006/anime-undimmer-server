...
def calculate_epilepsy_risk(frame_values_gen, frame_values_gen_2, range_max_values, range_avg_values, range_diff_values,
                            dim_multiplier):
    """
...
def calculate_max_without_outliers_single_frame(frame):
    ...
    max_no_outliers_frame = np.max(threshold_frames,
                                   axis=(0, 1))  # Max across height and width, but keep the RGB channels separate
...
def process_clip(clip):
    ...
    for i, (t, frame) in tqdm.tqdm(enumerate(clip.iter_frames(with_times=True)), total=clip.fps * clip.duration,
                                   dynamic_ncols=True, desc="Processing frames"):
        if prev_frame is None:  # To align the size of the arrays
            prev_frame = frame
...
def process_clip_parallel_modal_old(clip):
    ...
    results = Parallel(20)(delayed(process_frame)(frame, frames[i - 1][1] if i > 0 else frame)
                           for i, (t, frame) in
                           tqdm.tqdm(enumerate(frames), total=clip.fps * clip.duration, dynamic_ncols=True))
...
def find_dark_and_dimmed_ranges(max_values, threshold):
    ...
    if count >= 15:
        dark_and_dimmed_ranges.append(
            (start_time, len(max_values)))  # Add the last range if it ends at the end of the video
    end_timer = time.time()
...
def filter_and_print_range_characteristics(clip, max_values, avg_values, diff_values, dark_and_dimmed_ranges):
    ...
    for start, end in dark_and_dimmed_ranges:
        ...
        result = get_and_print_single_range_characteristics(clip, start, end, range_max_values, range_avg_values,
                                                            range_diff_values)
        if result:
...
def get_and_print_single_range_characteristics(clip, start, end, range_max_values, range_avg_values, range_diff_values):
    ...
    is_epileptic = calculate_epilepsy_risk(exact_frame_values, exact_frame_values_2, range_max_values, range_avg_values,
...
def get_dimmed_scenes(input_file, show_plot, threshold):
    ...
    if show_plot:
        plot_values(max_values_per_n_frames, avg_values_per_n_frames, max_no_outliers_values_values_per_n_frames,
                    n_frames)
        return []
    ...
    dimmed_ranges = filter_and_print_range_characteristics(clip, max_values, avg_values, diff_values,
                                                           dark_and_dimmed_ranges)
    return dimmed_ranges
...
def main():
    ...
    parser.add_argument('--modal', action='store_true',
                        help='Pass if should run the expensive parallel processing on modal in the cloud for speed')
    parser.add_argument('--only_plot', action='store_true', help='Only plot max frame value for each quarter second')
    parser.add_argument('--custom_scene', nargs=3, metavar=('start', 'end', 'factor (0 to auto-calculate)'),
                        help='Define a custom dimmed scene with start time, end time (in minutes:seconds or seconds), and optional dim factor (if 0, we will auto-calculate)')
